...
export class Board {
  constructor(gridRow, gridCol, numBombs) {
  ...
  onBoard(pos) {
    return (
      pos[0] >= 0 && pos[0] < this.gridRow &&
        pos[1] >= 0 && pos[1] < this.gridCol
    );
  }
  ...
  plantBombs() {
    ...
    while (totalPlantedBombs < this.numBombs) {
      const row = Math.floor(Math.random() * (this.gridRow - 1));
      const col = Math.floor(Math.random() * (this.gridCol - 1));
